...
- [ ] `GOTOOLCHAIN=go1.26.7 GOEXPERIMENT=jsonv2 go test ./... ./broadcast/... ./datastartest/... ./static/... -race -count=1` — all green
- [ ] `GOTOOLCHAIN=go1.26.7 GOEXPERIMENT=jsonv2 go vet ./... ./broadcast/... ./datastartest/... ./static/...` — clean
- [ ] `GOTOOLCHAIN=go1.26.7 GOEXPERIMENT=jsonv2 golangci-lint run ./... ./broadcast/... ./datastartest/... ./static/...` — 0 issues
...
- [ ] Bump versions in all 4 `go.mod` files via `go mod edit -version` (never sed)
- [ ] Verify all 4 modules have the same version (lockstep)
