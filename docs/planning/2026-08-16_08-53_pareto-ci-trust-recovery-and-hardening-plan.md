...
| ---- | ----- | ------ | ------ | ------ | ---------- | -------- | ------ |
| T01 | Green master CI: toolchain 1.26.6 + erraudit job disposition | 1% | Critical | 30min | Owner Q1 + Q2 | CI | 🔵 BLOCKED |
| T02 | Hygiene one-liners pack (result symlink, ROADMAP/AGENTS notes, nix flake check) | 1% | High | 30min | — | Repo | Ready |
| T03 | Formatter & lint wiring (dprint disposition, actionlint step, erraudit devShell) | 4% | High | 45min | — | Tooling | Ready |
| T04 | Public-trust pack (branch protection, pkg.go.dev verify, coverage badge) | 4% | High | 30min | T01 (CI green first) | Repo | Ready |
| T05 | ADR 002: multi-module split + mutual-replace pattern | 4% | Medium | 30min | — | Docs | Ready |
| T06 | CONTRIBUTING.md multi-module development section | 4% | Medium | 30min | — | Docs | Ready |
| T07 | `CollectPost` error-path tests | 20% | High | 30min | — | Testing | Ready |
| T08 | e2e dogfood expansion (`CollectPost`, `CollectN`) | 20% | Medium | 30min | — | Testing | Ready |
| T09 | Example `WithOnDrop` integration test | 20% | Medium | 30min | — | Testing | Ready |
| T10 | `UnmarshalSignals` fuzz test | 20% | Medium | 30min | — | Testing | Ready |
| T11 | Per-module Nix hermetic checks | 20% | Medium | 60min | — | CI | Ready |
| T12 | README comparison maintenance + release checklist | 20% | Medium | 30min | — | Docs | Ready |
| T13 | De-flake `TestCollect_WithLastEventID_HeaderArrives` | 20% | Medium | 30min | Coordinate with parallel session (G1) | Testing | Ready |
| T14 | `docs/modularization/README.md` index | Rest | Low | 30min | — | Docs | Ready |
| T15 | DOMAIN_LANGUAGE.md execution (create or record waiver) | Rest | Low | 30min | Owner Q3 | Docs | 🔵 BLOCKED |
| T16 | Owner-ruling execution pack (go.work.sum, v0.0.0 siblings, directive doc) | Rest | Medium | 30min | Owner rulings | Modules | 🔵 BLOCKED |
...
1. **Go directive:** bump all directives + CI to **1.26.6** (greens CI, clears 4 stdlib CVEs, supersedes the 1.26-vs-1.26.5 ghost) — yes/no?
2. **erraudit CI job:** gate it, silence it, or keep the red X until erraudit is public?
3. **DOMAIN_LANGUAGE.md:** must-have glossary, or waived for a protocol library?
