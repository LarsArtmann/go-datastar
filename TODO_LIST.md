...
| broadcast API ergonomics tranche (from the 2026-09-18 review): `NewBroadcasterWithStore(sse.EventStore)` for pluggable replay backends (Redis/Postgres per store.go's own guidance), close the buffer-size × replay constructor-matrix gap, optional heartbeat interval.                                | 🔴 `TODO` | Medium | 1h     | `docs/status/2026-09-18_19-54_broadcast-code-review.md` (R1–R3)                    |
