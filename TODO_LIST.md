...
| Watch the first runs of the new CI workflows and promote/act: `fuzz.yml` nightly + first CodeQL results + `nix.yml` promotion after a green week (drop `continue-on-error`); evaluate `upload-artifact` major then; verify the Renovate custom-manager regex maps `static/static.go` Version to real upstream tags on first run. | 🔴 `TODO` | Low    | 15min  | 20-12 report f.12, f.14-f.16, f.42, f.44; `nix.yml`, `fuzz.yml`, `codeql.yml`, `renovate.json` |
