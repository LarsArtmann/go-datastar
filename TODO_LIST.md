...
| Post-v0.4.0 watch: first `fuzz.yml` nightly + first CodeQL results + `nix.yml` promotion after a green week (drop `continue-on-error`); evaluate `upload-artifact` major then. | 🔴 `TODO` | Low    | 15min  | 20-12 report f.12, f.15, f.16, f.42, f.44; TODO watch rows                                      |
